function render_motion_task2(movieVersion)
...
for monkeyInd=1
    ...
    if drawFrames==1
        if monkeyInd==1
            switch(movieVersion)
                case 1
                    datapath = 'D:\data\041217_B25\eye_data_041217_B25.mat';%setNo 14
                case 2
                    datapath = 'D:\data\041217_B24\eye_data_041217_B24.mat';%setNo 15
                case 3
                    datapath = 'D:\data\041217_B35\eye_data_041217_B35.mat';%setNo 17
            end
        end
        load(datapath)
...
%                 end
                ...
                if thisDP/TrueSampleRate >= (300+150*3+100)/1000 && thisDP/ TrueSampleRate <= (300+150*3+100+340)/1000&& targetAcquiredFlag==0
                    plot(0,-200,'.','color',[0 0 0],'markersize',20)%draw targets on screen (upper left). Not to scale (actually separated by 10 pixels edge to edge, not 15, and actually 10 pixels in diameter, not 20
                    plot(0,200,'.','color',[0 0 0],'markersize',20)%draw targets on screen (upper right)
                end
...
%                 FrameData(:,:,:,totalFrame) = X;
        ...
        pathname=['D:\data\motion_video_in_setup\setup 1024-channel ori',num2str(movieVersion),'.mat'];
    ...
    if makeIndividualMovies==1
        ...
        moviename=['D:\data\motion_video_in_setup\setup 1024-channel ori',num2str(movieVersion),'.avi'];
        ...
        moviename=['D:\data\motion_video_in_setup\setup 1024-channel ori',num2str(movieVersion)];
