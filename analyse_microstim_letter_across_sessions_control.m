function analyse_microstim_letter_across_sessions_control(date)
%15/8/19
...
visualOnly=1;
load('X:\best\120819_data\electrodeSets120819.mat')
for calculateVisual=[0 1]
    for setInd=setNos%numSets%10 electrodes per letter
        setElectrodes=[allElectrodeSets(setInd,1) allElectrodeSets(setInd,2)];
        setArrays=[allArraySets(setInd,1) allArraySets(setInd,2)];
        perfNEV=[];
        timeInd=[];
        encodeInd=[];
        microstimTrialNEV=[];
...
%                         end
                ...
                if calculateVisual==0
                    ...
                    if ~isempty(perfMicroBin)
                        ...
                        save(['D:\data\perf_mat\microPerf_',date,'.mat'],'perfMicroBin');
                    end
                elseif calculateVisual==1
                    ...
                    if ~isempty(perfVisualBin)
                        ...
                        save(['D:\data\perf_mat\visualPerf_',date,'.mat'],'perfVisualBin');
                    end
                end
                    ...
                    for setNo=1:length(setElectrodes)
                        ...
                        for electrodeCount=1:length(setElectrodes{setNo})
                            ...
                            for electrodePairInd=1:size(electrodePairs,2)-1
                                ...
                                electrodeIndtemp1=find(goodArrays8to16New(:,8)==electrode1);%matching channel number
                                electrodeIndtemp2=find(goodArrays8to16New(:,7)==array1);%matching array number
                                electrodeInd1=intersect(electrodeIndtemp1,electrodeIndtemp2);%channel number
                                electrodeIndtemp1=find(goodArrays8to16New(:,8)==electrode2);%matching channel number
                                electrodeIndtemp2=find(goodArrays8to16New(:,7)==array2);%matching array number
                                electrodeInd2=intersect(electrodeIndtemp1,electrodeIndtemp2);%channel number
                                RFx1=goodArrays8to16New(electrodeInd1,1);
                                RFy1=goodArrays8to16New(electrodeInd1,2);
                                RFx2=goodArrays8to16New(electrodeInd2,1);
                                RFy2=goodArrays8to16New(electrodeInd2,2);
...
sprintf(['t(',num2str(stats.df),') = ',num2str(stats.tstat),', p = %.4f'],p)%t(9) = 0.1116, p = 0.9136
...
sprintf(['t(',num2str(stats.df),') = ',num2str(stats.tstat),', p = %.4f'],p)%t(9) = -1.1369, p = 0.2849
...
% ax.YTick=[0 3 6];
% 
% load('D:\aston_data\control_letter_behavioural_performance_all_sets_160719_B6_100trials.mat')
% subplot(1,2,2);
% edges=0:0.1:1;
% setNos=[1:7 9:13];
% numSets=length(setNos);
