...
%                 scatter(-roughPosIndX,-roughPosIndY,[],cols(arrayColInd,:),'MarkerFaceColor',cols(arrayColInd,:));
                ...
                if min(trialDataXSmooth{trialCounter})>5500&&max(trialDataYSmooth{trialCounter})<1500%exclude extreme outliers
                    try
%                         figHistogram=figure;
                        hX=histcounts(trialDataXSmooth{trialCounter},numHistBins);
                        [b1 b2]=calculateSaccadeEndpoint([1:length(hX.Values)]',hX.Values');%return the midpoints of the peaks of the bimodal distribution, relative to histogram bins
                        if floor(b1)<hX.NumBins&&floor(b2)<hX.NumBins&&floor(b1)>0&&floor(b2)>0
                            saccadeEndXB1=hX.BinEdges(floor(b1))+(b1-floor(b1))*(hX.BinEdges(2)-hX.BinEdges(1));%calculate precise midpoint of distribution of eye position values
                            saccadeEndXB2=hX.BinEdges(floor(b2))+(b2-floor(b2))*(hX.BinEdges(2)-hX.BinEdges(1));%calculate precise midpoint of distribution of eye position values
                        ...
                        hY=histcounts(trialDataYSmooth{trialCounter},numHistBins);
                        [b1 b2]=calculateSaccadeEndpoint([1:length(hY.Values)]',hY.Values');
                        if floor(b1)<hY.NumBins&&floor(b2)<hY.NumBins&&floor(b1)>0&&floor(b2)>0
                            saccadeEndYB1=hY.BinEdges(floor(b1))+(b1-floor(b1))*(hY.BinEdges(2)-hY.BinEdges(1));%calculate precise midpoint of distribution of eye position values
                            saccadeEndYB2=hY.BinEdges(floor(b2))+(b2-floor(b2))*(hY.BinEdges(2)-hY.BinEdges(1));%calculate precise midpoint of distribution of eye position values
