function render_impedance_time
...
for monkeyInd=1
    if monkeyInd==1%Lick
        ...
        rootDir='D:\data';
        remoteDir='X:\best';
        load('D:\data\signal_quality\impedance_170419.mat');
    ...
    elseif monkeyInd==2%Aston
        ...
        remoteDir='X:\aston';
        load('D:\aston_data\signal_quality\impedance_160819.mat');
    ...
    %read in impedance data
    drawFrames=1;
    if drawFrames==1
        impedanceThreshold=200;
        frameNo=1;
        for timePoint=1:size(impedanceAllChannelsSessions,2)
...
%             badInd=find(allSNR(:,timePoint)<SNRthreshold);
            col=impedanceAllChannelsSessions(:,timePoint);
            col(col>impedanceThreshold)=impedanceThreshold;
%             col(badInd)=0;
%             col(goodInd1)=1;
%             col(goodInd2)=2;
%             col(goodInd3)=3;
            newcol=255*col/impedanceThreshold;
...
%             newcol=newcol*255/(255-90);
        ...
        pathname=[rootDir,'\signal_quality\impedance_over_time_cortex.mat'];
    ...
    if makeIndividualMovies==1
        ...
        moviename=[rootDir,'\signal_quality\impedance_over_time_cortex.avi'];
        ...
        moviename=[rootDir,'\signal_quality\impedance_over_time_cortex'];
