function render_orientation_task2
%Written by Xing 1/4/20. Read in images of monkeys in setup, identify
...
for monkeyInd=1
    ...
    if drawFrames==1
        if monkeyInd==1
            datapath = 'D:\data\231017_B11\eye_data_231017_B11.mat';
        end
        load(datapath)
...
%             thisCond = targetLocationsFinal(thisTrialIdx)
            ...
            for thisDP = 1:TrueSampleRate/movieFrameRate:numDP%30 kHz/1000 = 30 fps
                copyA2=copyA;
                hold off
...
%                 end
                ...
                if thisDP/TrueSampleRate >= 300/1000 && thisDP/ TrueSampleRate <= (300+167)/1000
                    plot(elecRFs.x*pixperdeg,elecRFs.y*pixperdeg,'.','color',[1 1 1],'markersize',15)%draw phosphenes on screen
                end
                % plot targets
                if thisDP/TrueSampleRate >= (300+400)/1000 && thisDP/ TrueSampleRate <= (300+400+340)/1000
                    plot(0,200-15,'.','color',[0 0 0],'markersize',20)%draw targets on screen (upper left). Not to scale (actually separated by 10 pixels edge to edge, not 15, and actually 10 pixels in diameter, not 20
                    plot(0,200+15,'.','color',[0 0 0],'markersize',20)%draw targets on screen (upper right)
                    plot(-15,-200,'.','color',[0 0 0],'markersize',20)%draw targets on screen (lower left)
                    plot(15,-200,'.','color',[0 0 0],'markersize',20)%draw targets on screen (lower right)
                end
                %                 image(copyA2);
...
%                 FrameData(:,:,:,totalFrame) = X;
        ...
        pathname=['D:\data\ori_video_in_setup\setup 1024-channel ori',num2str(movieVersion),'.mat'];
    ...
    if makeIndividualMovies==1
        ...
        moviename=['D:\data\ori_video_in_setup\setup 1024-channel ori',num2str(movieVersion),'.avi'];
        ...
        moviename=['D:\data\ori_video_in_setup\setup 1024-channel ori',num2str(movieVersion)];
