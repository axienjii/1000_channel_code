function render_orientation_task
%Written by Xing 28/5/19. Read in images of monkeys in setup, identify
...
for monkeyInd=1
    ...
    if selectScreenCorners==1
        ...
        load('D:\data\ori_video_in_setup\lick_screenCornerCoords.mat')
    end
    ...
    if monkeyInd==1
        cd W:\Feng\behaviormovies\2-phosphene_task_movie        
...
%                 imshow(B);
                ...
                if thisDP/TrueSampleRate >= 300/1000 && thisDP/ TrueSampleRate <= (300+167)/1000
                    ...
                    hold on
%                     plot(400+elecRFs.x*40,800+elecRFs.y*40,'.','color',[1 1 1],'markersize',30)% below screen
%                     plot(1100+elecRFs.x*40,300+elecRFs.y*40,'.','color',[1 1 1],'markersize',30)%top left corner
                    for phospheneInd=1:numElec
                        ms=15;
                        ...
                        for rgbInd=1:3%for images, x and y coordinates are swapped
                            copyA2(800+floor(elecRFs.y(phospheneInd)*40):800+floor(elecRFs.y(phospheneInd)*40)-1+size(maskblob,2),400+floor(elecRFs.x(phospheneInd)*40):400+floor(elecRFs.x(phospheneInd)*40)-1+size(maskblob,1),rgbInd)=maskblob;
                        end
                    end
                end
                image(copyA2);
...
%                 FrameData(:,:,:,totalFrame) = X;
        ...
        pathname=[rootDir,'\ori_video_in_setup\setup 1024-channel ori.mat'];
        save(pathname,'framesResponse','-v7.3')
    end
        
    % write to movie
    myVideo = VideoWriter(['./ori_movie',num2str(1),'_XC.mp4'],'MPEG-4');
...
%         thisFrameData = squeeze(framesResponse(:,:,:,thisframe));
    ...
    if makeIndividualMovies==1
        ...
        moviename=[rootDir,'\ori_video_in_setup\setup 1024-channel ori.avi'];
        ...
        for timePoint=1:length(framesResponse)
            if size(framesResponse(timePoint).cdata,1)~=917||size(framesResponse(timePoint).cdata,2)~=1344
                framesResponse(timePoint).cdata=framesResponse(timePoint).cdata(1:917,1:1344,:);
        ...
        moviename=[rootDir,'\ori_video_in_setup\setup 1024-channel ori'];
