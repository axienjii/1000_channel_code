function render_letter_task_multi
...
for monkeyInd=1
    ...
    if monkeyInd==1
        ...
        thisMovie = 1;
        targetLocationsFinal(trialBlockIdx(:,sortedtrialblockidx(thisMovie)))
        perfNEVFinal(trialBlockIdx(:,sortedtrialblockidx(thisMovie)))
    end
    ...
    if drawFrames==1
        ...
        for thisTrial = 1:NumTotalTrialsInMovie
            ...
            arrays = allArrayNumFinal{thisTrialIdx};
                        ...
                        for rgbInd=1:3%for images, x and y coordinates are swapped
                            for xInd=1:size(maskblob,1)
                                for yInd=1:size(maskblob,2)
                                    if copyA2(600+floor(elecRFs.y(thisStimElec)*-40)+yInd-1,250+floor(elecRFs.x(thisStimElec)*40)+xInd-1,rgbInd)==0
                                        copyA2(600+floor(elecRFs.y(thisStimElec)*-40)+yInd-1,250+floor(elecRFs.x(thisStimElec)*40)+xInd-1,rgbInd)=maskblob(xInd,yInd);
                                    else
                                        copyA2(600+floor(elecRFs.y(thisStimElec)*-40)+yInd-1,250+floor(elecRFs.x(thisStimElec)*40)+xInd-1,rgbInd)=max(copyA2(600+floor(elecRFs.y(thisStimElec)*-40)+yInd-1,250+floor(elecRFs.x(thisStimElec)*40)+xInd-1,rgbInd)+maskblob(xInd,yInd));
...
%                 FrameData(:,:,:,totalFrame) = X;
        ...
        pathname=[rootDir,'\letter_video_in_setup\setup 1024-channel letter multi.mat'];
        save(pathname,'framesResponse','-v7.3')
    end
        
    % write to movie
    myVideo = VideoWriter(['./letter_movie_multi',num2str(1),'_XC.mp4'],'MPEG-4');
