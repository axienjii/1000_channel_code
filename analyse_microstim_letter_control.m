function analyse_microstim_letter_control(date,allInstanceInd)
%15/8/19
%Written by Xing, modified from analyse_microstim_line.m, calculates behavioural performance during a
%microstimulation/visual control 'shape' task.
...
if analyseConds==1
    switch(date)
        %microstim task only:
        case '130819_B7';%novel electrode combinations
            setInd=1;
            numTargets=2;
        ...
        case '150819_B2'
            ...
            visualOnly=0;
            %visual task only:
        case '130819_B5';%novel electrode combinations
            setInd=1;
            numTargets=2;
            electrodePairs=[1:length(setElectrodes{1});1:length(setElectrodes{2})];
            currentThresholdChs=141;
