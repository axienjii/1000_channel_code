...
allElectrodeAllTrialsMin=[];
allArrayAllTrialsMin=[];
...
%             end
            if extractCurrentOnly==1
                fileName=[rootdir,date,'\saccade_data_',date,'_fix_to_rew_max_mid_amp_list.mat'];
%                     if suprathresholdCurrent==1
...
%                     if exist('proportionc50Max','var')
                        if length(allElectrodeNumsList)==length(allMaxCurrentList)%&&length(allElectrodeNumsList)==length(proportionc50Max)
%                             proportionc50Max(indRemove1)=[];
                            maxElectrodesArrays=[allElectrodeNumsList' allArrayNumsList'];
                            ...
                            allElectrodeAllTrialsMax=[allElectrodeAllTrialsMax allElectrodeNumsList(uniqueInd)];
                            allArrayAllTrialsMax=[allArrayAllTrialsMax allArrayNumsList(uniqueInd)];
%                             allProportionc50Max=[allProportionc50Max proportionc50Max(uniqueInd)];
                        end
%                     end
                    %                 elseif suprathresholdCurrent==0
%                     if exist('proportionc50Mid','var')
                        if length(allElectrodeNumsList)==length(allMidCurrentList)%&&length(allElectrodeNumsList)==length(proportionc50Mid)
%                             proportionc50Mid(indRemove1)=[];
                            minElectrodesArrays=[allElectrodeNumsList' allArrayNumsList'];
                            ...
                            allElectrodeAllTrialsMid=[allElectrodeAllTrialsMid allElectrodeNumsList(uniqueInd)];
                            allArrayAllTrialsMid=[allArrayAllTrialsMid allArrayNumsList(uniqueInd)];
...
arrayNums(indRemove)=[];
if extractCurrentOnly==1
    emptyInd=find(isempty(allPosIndXChsUnique));
    allMaxCurrentUnique(emptyInd)=[];
    allMidCurrentUnique(emptyInd)=[];
%     allProportionc50MaxUnique(emptyInd)=[];
%     allProportionc50MidUnique(emptyInd)=[];
end
...
grandMeanProportionc50Max=nanmean(meanProportionc50Max)
grandStdProportionc50Max=nanstd(meanProportionc50Max)
grandMeanProportionc50Max=nanmean(meanProportionc50Mid)
grandStdProportionc50Max=nanstd(meanProportionc50Mid)
...
%calculate mean number of trials and SD per electrode:
allPosIndXChsUniqueMaxFinal=allPosIndXChsUniqueMax(indMax);
allPosIndYChsUniqueMaxFinal=allPosIndYChsUniqueMax(indMax);
allPosIndXChsUniqueMidFinal=allPosIndXChsUniqueMax(indMid);
allPosIndYChsUniqueMidFinal=allPosIndYChsUniqueMax(indMid);
for finalElectrodeInd=1:length(indMax)
    numTrialsMax(finalElectrodeInd)=sum(~isnan(allPosIndXChsUniqueMaxFinal{finalElectrodeInd}));
    numTrialsMin(finalElectrodeInd)=sum(~isnan(allPosIndXChsUniqueMidFinal{finalElectrodeInd}));
end
meanTrialsMax=mean(numTrialsMax)
meanTrialsMin=mean(numTrialsMin)
SDTrialsMax=std(numTrialsMax)
SDTrialsMin=std(numTrialsMin)
