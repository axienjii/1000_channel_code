function compare_letter_task_control_100trials
%Written by Xing 17/7/19.
...
for calculateVisual=[0 1]
    for setNo=[1:6 9 17:24 26 28:29]%1:29%[1:11 13:26 28:29]%1:24 for 10 electrodes per letter; subsequent sessions use 15 electrodes per letter
        if calculateVisual==0
            ...
            meanPerf100TrialsM=[meanPerf100TrialsM mean(perfMicroBin)];
        elseif calculateVisual==1
            ...
            meanPerf100TrialsV=[meanPerf100TrialsV mean(perfVisualBin)];
        end
...
sprintf(['t(',num2str(stats.df),') = ',num2str(stats.tstat),', p = %.4f'],p)%t(17) = 19.4672, p = 0.0000
...
sprintf(['t(',num2str(stats.df),') = ',num2str(stats.tstat),', p = %.4f'],p)%t(26) = -9.6728, p = 0.0000
[h,p,ci,stats]=ttest2(meanPerfAllSets,meanPerf100TrialsM)
sprintf(['t(',num2str(stats.df),') = ',num2str(stats.tstat),', p = %.4f'],p)%t(26) = -3.0657, p = 0.0050
...
for calculateVisual=[0 1]
    for setNo=3:12
        if calculateVisual==0
            ...
            load(['D:\aston_data\perf_mat\microPerf_',date,'.mat']);
            meanPerf100TrialsM=[meanPerf100TrialsM mean(perfMicroBin)];
        elseif calculateVisual==1
            switch(setNo)
                %visual task only:
            ...
            load(['D:\aston_data\perf_mat\visualPerf_',date,'.mat']);
            meanPerf100TrialsV=[meanPerf100TrialsV mean(perfVisualBin)];
        end
