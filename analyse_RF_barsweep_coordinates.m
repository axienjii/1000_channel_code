...
for instanceInd=8:8
    ...
    for channelInd=1:128
        for stimCond = 1:4
            ...
            seedParams = [midpoint 100 Scale 0];%midpoint, std (in ms), difference between peak and baseline, and baseline
            px=1:length(mua2fit);
        ...
        hold on
        scatter(0,0,'r','f')
        if mean(channelSNR(channelInd,:))>4
        ...
        hold on
        if mean(channelSNR(channelInd,:))>4
            scatter(RF.centrex(1),RF.centrey(1),20,'k','x')
            ellipse(abs(right-left)/2,abs(top-bottom)/2,RF.centrex(1),RF.centrey(1));
        else
            scatter(RF.centrex(1),RF.centrey(1),20,[1 0 0],'x')
        end
