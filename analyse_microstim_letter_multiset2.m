function analyse_microstim_letter_multiset2(date,allInstanceInd)
%16/5/18
%Written by Xing, modified from analyse_microstim_letter_multiset.m, calculates behavioural performance during a
...
%Task involves exclusively T vs O.
            ...
            for condNo=1:length(setInds)
                ...
                for setNo=1:2
                    ...
                    if ~isempty(firstTrials)
                        condInds{setNo}=intersect(condInds{setNo},responseTrialsInd);
                        ...
                        corrIndsV{setNo}=intersect(condInds{setNo}(1:firstTrials),correctVisualTrialsInd);
                        incorrIndsV{setNo}=intersect(condInds{setNo}(1:firstTrials),incorrectVisualTrialsInd);
                    else
...
%                     end
                ...
                %            if numTargets==4
                if setNo==1
                    subplot(4,5,condNo+5);
                elseif setNo==2
                    subplot(4,5,condNo+15);
                end
%                 if figureSubPlotNum<=5
%                     subplot(4,5,figureSubPlotNum+5);
%                 else
