function analyse_microstim_2phosphene_eye(date,allInstanceInd)
%3/1/19
%Written by Xing, modified from analyse_microstim_letter_eye2.m, extracts eye data during a
%microstimulation/visual 2-phosphene orientation discrimination task.
%Used on recording 231017_B42: analyse_microstim_2phosphene_eye('231017_B42',1)
...
%             length(targetLocTrials)
        ...
        for trialInd=1:length(goodTrialsInd)
            allArrayNumFinal{trialInd}=[allArrayNum(goodTrialsInd(trialInd)) allArrayNum2(goodTrialsInd(trialInd))];
            allElectrodeNumFinal{trialInd}=[allElectrodeNum(goodTrialsInd(trialInd)) allElectrodeNum2(goodTrialsInd(trialInd))];
        end
