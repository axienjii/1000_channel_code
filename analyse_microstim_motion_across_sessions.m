...
for calculateVisual=[0 1]
    for setNo=[1 3:12 14:17]%[1 3:6 8 10:12 14:17]%before RF correction:[1 3:12 14:17]%[1:17]%26%sets 2 and 13 have relatively few microstim trials
        perfNEV=[];
        timeInd=[];
        encodeInd=[];
        microstimTrialNEV=[];
...
%                         end
                ...
                initialPerfTrials=41;%41;%first set of trials are the most important
                if calculateVisual==0
...
% set(gcf,'PaperPositionMode','auto','Position',get(0,'Screensize'))
% print(pathname,'-dtiff');
pathname=fullfile(['D:\data\behavioural_performance_all_sets_041217_',num2str(initialPerfTrials),'trials_highres_corrected_RFs']);
set(gcf,'PaperPositionMode','auto','Position',get(0,'Screensize'))
print(pathname,'-dtiff');
...
perfMat=['D:\data\behavioural_performance_all_sets_041217_',num2str(initialPerfTrials),'trials_corrected_RFs.mat'];
save(perfMat,'meanAllSetsPerfVisualBin','meanAllSetsPerfMicroBin');
pause=1;
