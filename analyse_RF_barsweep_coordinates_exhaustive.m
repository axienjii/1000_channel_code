function analyse_RF_barsweep_coordinates_exhaustive(date)
%3/12/19
%Written by Xing, modified from analyse_RF_barsweep_coordinates.m, but uses
%Matt's newly developed method of performing an exhaustive search using
%many possible combinations of SD width and midpoint of the fitted
%Gaussian, to optimise the fit, avoid local minima, and obtain RF
%coordinates from 1000-channel data. 
%Analyses data generated by analyse_RF_barsweep.m, collected during
%presentation of bar sweep stimuli using runstim_RF_barsweep.
        ...
        for stimCond = 1:4
            ...
            for i = 1:length(pxm)
                for j = 1:length(sdind)
                    ...
                    diff = sum((model-MUAm(RespT)).^2);
                    z = z+1;
                    ind(z,:) = [diff,pxm(i),sdind(j)];
                end
            end
            [x,y] = min(ind(:,1));
            ModM(stimCond) = ind(y,2);
            ModS(stimCond) = ind(y,3);
            Ons(1,stimCond) = ModM(stimCond)-(1.95.*ModS(stimCond)*100);
            Offs(1,stimCond) = ModM(stimCond)+(1.95.*ModS(stimCond)*100);
            %Best model
            norm = normpdf(td, ModM(stimCond), ModS(stimCond)*100);
            norm = norm./(max(norm));
...
%             Offs(1,stimCond) = params(1)+(1.65.*params(2));
            ...
            plot(bestmodel-Base,stimCondCol(stimCond),'Color',[0.5 0.5 0.5]);
            ax=gca;
...
%             scatter(RF.centrex(1),RF.centrey(1),20,[1 0 0],'x')
    ...
    pathname=fullfile('X:\best',date,[instanceName,'_RFs_SNR',num2str(SNRthreshold),'exhaustive']);
    if plotAreas==0
        pathname=fullfile('X:\best',date,[instanceName,'_RFs_coords_only_SNR',num2str(SNRthreshold),'exhaustive']);
    end
    ...
    if printStimCondResponses==1
        for figInd=1:4
            figure(figInd)
            set(gcf,'PaperPositionMode','auto','Position',get(0,'Screensize'))
            pathname=fullfile('X:\best',date,[instanceName,'_',num2str(figInd),'exhaustive']);
            print(pathname,'-dtiff');
        end
    end
%     for figInd=1:4
...
%     end
    ...
    length(countGoodSNR)
    fileName=fullfile('X:\best',date,['RFs_',instanceName,'exhaustive.mat']);
    saveFile=1;
    if saveFile==1
