function analyse_microstim_motion_eye(date,allInstanceInd)
%3/1/19
%Written by Xing, modified from analyse_microstim_letter_eye2.m, extracts eye data during a
%microstimulation/visual task.
%Used on recording 071217_B28: analyse_microstim_motion_eye('071217_B28',1)
...
stimDurms=167*5;%in ms- min 0, max 500
preStimDur=300/1000;
...
%             NSchNew1=[];
%             NSchNew2=[];
%             for cellInd=1:length(NSch)
%                 NSchNew1=[NSchNew1 NSch{1}{cellInd}];
%                 NSchNew2=[NSchNew2 NSch{2}{cellInd}];
%             end
%             NSch{1,1}=NSchNew1;
%             NSch{1,2}=NSchNew2;
            NSchNew{1,1}=NSch{1}{1,2};
            NSchNew{1,2}=NSch{2}{1,2};
            NSch=NSchNew;
        end
...
%             length(targetLocTrials)
        ...
        trimmean(allY{3},95);
        trimmean(allY{4},95);
        allElectrodeNumFinal=cell(1,length(goodTrialsInd));
        allElectrodeNumFinal=cell(1,length(goodTrialsInd));
        for trialInd=1:length(goodTrialsInd)
            allArrayNumFinal{trialInd}=[allArrayNum(goodTrialsInd(trialInd))];
            allElectrodeNumFinal{trialInd}=[allElectrodeNum(goodTrialsInd(trialInd))];
        end
