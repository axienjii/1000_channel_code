function analyse_microstim_saccade_current(date,allInstanceInd)
%18/10/18
...
%the expected phosphene locations, based on RF mapping, and whether they
%vary as a function of current amplitude. Present 10 conditions of varying
%current amplitude, with 10 trials per condition, to check for correlation
%between eccentricity of saccade end point, and current amplitude.
...
if processRaw==1
    for instanceCount=1%:length(allInstanceInd)
        ...
        for uniqueCurrent=1:length(currentValsDesired)%15%16:30%1:15%
            arrayInd=find(cell2mat(allArrayNum)==array);
            figInd9(uniqueCurrent)=figure;hold on
            currentAmp=currentValsDesired(uniqueCurrent);
            currentCondTrials=find(allCurrentLevel==currentAmp);
            ...
            matchTrials=intersect(matchTrials,currentCondTrials);%identify subset of trials where a particular current amplitude was delivered
            performanceCurrentCond(uniqueCurrent)=length(matchTrials)/trialsDesired;%10 trials per condition
            trialDataXY={};
...
%             degPerVoltYFinal=0.0022;
            ...
            if ~isempty(matchTrials)
                for trialCounter=1:length(matchTrials)%for each correct microstim trial
                    ...
                    text(posIndX-0.05,-posIndY,[num2str(electrode),'(',num2str(currentAmp),')'],'FontSize',6,'Color',[impCol impCol 1]);
                    
                    figure(figInd9(uniqueCurrent))%plot the saccade end points for an individual electrode, with the mean
                    %                 subplot(11,10,uniqueCurrent);
                    hold on
                    plot(posIndX,-posIndY,'MarkerEdgeColor',cols(arrayColInd,:),'Marker','o','MarkerSize',10);
                    impCol=impedance*0.9/150+0.05;
                    text(posIndX-0.05,-posIndY,[num2str(electrode),'(',num2str(currentAmp),')'],'FontSize',6,'Color',[impCol impCol 1]);
                ...
                allMeanSaccadeXY(uniqueCurrent,:)=meanSaccadeXY;
                allStdSaccadeXY(uniqueCurrent,:)=stdSaccadeXY;
                ...
                text(meanSaccadeXY(1)-0.05,-meanSaccadeXY(2),[num2str(electrode),'(',num2str(currentAmp),')'],'FontSize',8,'Color',[0 0 0]);
                ...
                if numGoodTrials>0&&stdSaccadeXY(1)<stdCutoff&&stdSaccadeXY(2)<stdCutoff
                    ...
                    text(RFx+0.15,RFy,[num2str(electrode),'(',num2str(currentAmp),')'],'FontSize',7,'Color',[0 0 0]);
                ...
                ellipse(stdSaccadeXY(1),stdSaccadeXY(2),meanSaccadeXY(1),-meanSaccadeXY(2),cols(arrayColInd,:));
                plot(RFx,RFy,'MarkerEdgeColor',cols(arrayColInd,:),'Marker','o','MarkerSize',6);
                text(meanSaccadeXY(1)-0.07,-meanSaccadeXY(2),[num2str(electrode),'(',num2str(currentAmp),')'],'FontSize',8,'Color',[0 0 0]);
                text(RFx-0.07,RFy,[num2str(electrode),'(',num2str(currentAmp),')'],'FontSize',8,'Color',[0 0 0]);
                ...
                plot(RFx,RFy,'MarkerEdgeColor',cols(arrayColInd,:),'Marker','o','MarkerSize',6);
                text(meanSaccadeXY(1)+0.2,-meanSaccadeXY(2),[num2str(electrode),'(',num2str(currentAmp),')'],'FontSize',8,'Color',[0 0 0]);
                text(RFx+0.2,RFy,[num2str(electrode),'(',num2str(currentAmp),')'],'FontSize',8,'Color',[0 0 0]);
                plot([meanSaccadeXY(1) RFx],[-meanSaccadeXY(2) RFy],'-','Color',cols(arrayColInd,:));
                ...
                xlim([-20 160]);
                ylim([-160 20]);
                title(['saccade endpoints, current = ',num2str(currentAmp),'uA']);
                ...
                pathname=fullfile(rootdir,date,['saccade_endpoints_dva_array_',num2str(array),'_electrode_',num2str(electrode),' current = ',num2str(currentAmp),'uA','_',date]);
                print(pathname,'-dtiff');
                close(figInd9(uniqueCurrent))
                baselineXChs{uniqueCurrent}=baselineXTrials;
                baselineYChs{uniqueCurrent}=baselineYTrials;
                trialDataXSmoothChs{uniqueCurrent}=trialDataXSmooth;
                trialDataYSmoothChs{uniqueCurrent}=trialDataYSmooth;
                trialNoChs{uniqueCurrent}=trialNoTrials;
                posIndXChs{uniqueCurrent}=posIndXTrials;
                posIndYChs{uniqueCurrent}=posIndYTrials;
                trialDataXSmoothFixChs{uniqueCurrent}=trialDataXSmoothFix;
                trialDataYSmoothFixChs{uniqueCurrent}=trialDataYSmoothFix;
                startMicrostimChs{uniqueCurrent}=startMicrostim;
                endMicrostimChs{uniqueCurrent}=endMicrostim;
            end
        end
        ...
        %catch trials:
        catchTrials=find(allCurrentLevel==0);
        correctTrialsInd=find(performance==1);
        correctCatchTrialsInd=intersect(catchTrials,correctTrialsInd);
        matchTrials=intersect(correctCatchTrialsInd,catchTrials);
        performanceCatchCond=length(matchTrials)/length(catchTrials);%10 trials per condition

        %bar plot showing performance:
        figure
        b=bar([performanceCatchCond performanceCurrentCond]);%performance on catch trials, followed by performance on each current amplitude condition
        title(['performance vs current strength, array ',num2str(array),' electrode ',num2str(electrode)])
        xlabel('current amplitude (uA)');
        ylabel('performance');
        ylim([0 1]);
        ax=gca;
        ax.XTick=1:length(currentValsDesired)+1;
        ax.XTickLabel={[0 currentValsDesired]};
        hold on%change colour of first bar (corresponding to catch trials) by plotting new bar on top
        b=bar(performanceCatchCond);
        set(b(1),'facecolor',[0.5 0 0.5]) 
        set(gcf,'PaperPositionMode','auto','Position',get(0,'Screensize'))
        pathname=fullfile(rootdir,date,['performance_array_',num2str(array),'_electrode_',num2str(electrode),'_',date]);
        print(pathname,'-dtiff');
        
        %eccentricity as a function of current amplitude:
        for uniqueCurrent=1:length(currentValsDesired)
            eccentricity(uniqueCurrent)=sqrt(allMeanSaccadeXY(uniqueCurrent,1)^2+allMeanSaccadeXY(uniqueCurrent,2)^2)/Par.PixPerDeg;
        end    
        figure
        plot(currentValsDesired,eccentricity,'ko-');%performance on catch trials, followed by performance on each current amplitude condition
        title(['eccentricity vs current strength, array ',num2str(array),' electrode ',num2str(electrode)])
        xlabel('current amplitude (uA)');
        ylabel('eccentricity (dva)');
        ax=gca;
%         set(gcf,'PaperPositionMode','auto','Position',get(0,'Screensize'))
        pathname=fullfile(rootdir,date,['eccentricity_vs_current_array_',num2str(array),'_electrode_',num2str(electrode),'_',date]);
        print(pathname,'-dtiff');
